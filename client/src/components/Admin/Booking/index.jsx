...
function BookSlot() {
  ...
  function getSlotsDetails() {
    axios.get("http://localhost:3001/admin/getSlot").then(({ data }) => {
      // console.log(data[0]);
      setSlots(data[0]);
    });
  }
  ...
  const setis = () => {
    axios.get("http://localhost:3001/admin/getApprovedCompany").then((response) => {
  ...
  const slotSelected = () => {
    const data = {
      company: selectSlot,
      ...choosedSlot,
      isAllotted: setChoosedSlot ? true : false,
    };
    // console.log(data);
    axios.patch("http://localhost:3001/admin/bookSlot", data).then((response) => {
      // console.log(response.data);
